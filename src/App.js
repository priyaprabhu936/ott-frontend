...
function App() {
  ...
  const fetchMovies = async () => {
    try {
      ...
      console.error("Fetch movies failed:", e?.message || e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const resetForm = () => {
    setTitle(""); setDescription(""); setPoster(""); setRating(3); setCategory("General"); setEditId(null);
  };
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      alert(isAdmin ? "Save failed (check login/token)." : "You are not admin.");
      console.error(e);
    } finally {
      setSaving(false);
    }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      alert(isAdmin ? "Delete failed." : "You are not admin.");
    }
  };
  ...
  const doAuth = async (e) => {
    ...
    try {
      ...
      alert("Auth failed. Check email/password.");
      console.error(e);
    }
  };
  ...
  return (
    <div style={{ padding: 24, fontFamily: "Arial, sans-serif", background: "#f5f6f8", minHeight: "100vh" }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto 12px", display: "flex", gap: 12, alignItems: "center", justifyContent: "space-between" }}>
        ...
        {user ? (
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <span style={{ fontSize: 14, color: "#333" }}>Hello, <b>{user.name}</b> {isAdmin ? "(Admin)" : ""}</span>
      ...
      {/* Auth Cards (show if not logged in) */}
      {!user && (
        <form
          onSubmit={doAuth}
          style={{
            maxWidth: 480, margin: "0 auto 20px", background: "white", padding: 16,
        ...
        >
          <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
            <button type="button"
              ...
              style={{ flex: 1, padding: 10, borderRadius: 8, border: "1px solid #ddd", background: authMode === "login" ? "#e6f0ff" : "white" }}>
              Login
            </button>
            <button type="button"
              onClick={() => setAuthMode("register")}
              style={{ flex: 1, padding: 10, borderRadius: 8, border: "1px solid #ddd", background: authMode === "register" ? "#e6f0ff" : "white" }}>
              Register
            </button>
          </div>
          ...
          <div style={{ fontSize: 12, color: "#666", marginTop: 8 }}>
            Tip: First registered user becomes <b>Admin</b> automatically.
          </div>
        </form>
      )}
      ...
      {user && isAdmin && (
        <form
          onSubmit={handleSave}
          style={{
            maxWidth: 720, margin: "0 auto 24px", display: "grid", gap: 10, gridTemplateColumns: "1fr 1fr",
            background: "white", padding: 16, borderRadius: 12, border: "1px solid #eee", boxShadow: "0 4px 12px rgba(0,0,0,0.05)"
          }}
        >
          <input
            type="text" placeholder="Movie Title" value={title} onChange={(e) => setTitle(e.target.value)} required
          ...
          />
          <input
            type="text" placeholder="Poster URL" value={poster} onChange={(e) => setPoster(e.target.value)} required
            style={{ padding: 10, borderRadius: 8, border: "1px solid #ddd", gridColumn: "1 / span 2" }}
          />
          ...
          <input
            type="text" placeholder="Category (e.g. Action, Comedy)" value={category} onChange={(e) => setCategory(e.target.value)}
          ...
          <div style={{ gridColumn: "1 / span 2", display: "flex", gap: 10 }}>
            ...
            {editId && (
              <button type="button" onClick={resetForm} style={{ padding: "10px 14px", border: "1px solid #ccc", borderRadius: 8, background: "white" }}>
                ✨ Cancel Edit
              </button>
            )}
          </div>
      ...
      {!loading && filtered.length === 0 && <div style={{ textAlign: "center", color: "#666" }}>No movies found.</div>}

      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 16, maxWidth: 1100, margin: "0 auto" }}>
        {filtered.map((m) => (
          <div key={m._id} style={{ background: "white", border: "1px solid #eee", borderRadius: 12, overflow: "hidden", boxShadow: "0 4px 12px rgba(0,0,0,0.05)" }}>
            {m.poster ? (
              <img src={m.poster} alt={m.title} style={{ width: "100%", height: 220, objectFit: "cover" }} />
            ) : (
              <div style={{ height: 220, display: "flex", alignItems: "center", justifyContent: "center", background: "#f0f0f0" }}>No Poster</div>
            )}
            <div style={{ padding: 12 }}>
              ...
              {user && isAdmin ? (
                <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                  <button onClick={() => handleEdit(m)} style={{ flex: 1, padding: "8px 10px", border: "none", borderRadius: 8, background: "#ffc107", color: "white" }}>
                    ✏ Edit
                  </button>
                  <button onClick={() => handleDelete(m._id)} style={{ flex: 1, padding: "8px 10px", border: "none", borderRadius: 8, background: "#dc3545", color: "white" }}>
                    🗑 Delete
                  </button>
                </div>
              ) : null}
